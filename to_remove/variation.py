...
class Variations_Model():
    ...
    def __init__(self, seeds, file_writer, trainset, testset,
        ...
        self.seen_classes_means = {k:0 for k in range(100)}
        self.count_seen_classes= {k:0 for k in range(100)}
    ...
    def train(self, split):
        ...
        for e in range(self.epochs):
            ...
            for inputs, labels in self.train_dataloader:
                ...
                outputs = self.compute_class_score(inputs, labels)
                # compute the loss
    ...
    def online_mean(self, mapped_labels, batch_features, batch_labels, batch_counts):

        #batch_features = self.net.extract_features(batch_imgs)
        #batch_labels, batch_counts = np.unique(mapped_label.cpu().numpy(), return_counts=True)
        #self.current_feaures_variance = torch.var(batch_features, dim=0, unbiased=True)
        
        for cl_idx, cl in enumerate(batch_labels):

            # retrieve all the features of class cl
            mask = mapped_labels == cl
            masked_idxs = torch.where(mask)
            cl_batch_features = batch_features[masked_idxs]
            
            cl_batch_features_mean = torch.mean(cl_batch_features, dim=0)

            #if self.seen_classes_means[cl] is not None:
            mean = (self.count_seen_classes[cl] * self.seen_classes_means[cl] +\
                batch_counts[cl_idx] * cl_batch_features_mean) / \
                    (self.count_seen_classes[cl] + batch_counts[cl_idx])
            
            self.seen_classes_means[cl] = mean
            self.count_seen_classes[cl] += batch_counts[cl_idx]

    def compute_class_score(self, batch_imgs, mapped_labels):

        batch_features = self.net.extract_features(batch_imgs)
        batch_labels, batch_counts = np.unique(mapped_labels.cpu().numpy(), return_counts=True)
        self.current_feaures_variance = torch.var(batch_features, unbiased=True)

        self.online_mean(batch_imgs, mapped_labels, batch_features, batch_labels, batch_counts)
  
        scores = []
        
        for cl in batch_labels:
                                
            score = torch.exp(-1/self.current_feaures_variance * \
                torch.norm(batch_features - self.seen_classes_means[cl]))

            scores.append(score)

        scores = torch.stack(scores)
        score_denom = torch.sum(scores, dim=0)

        return (scores/score_denom).reshape(self.batch_size, -1).cuda()
